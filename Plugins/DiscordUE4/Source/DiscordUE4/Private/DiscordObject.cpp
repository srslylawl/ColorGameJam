...
void UDiscordObject::DestroyDiscordObject()
{
	...
	{
		DiscordObjectInstance->UpdateAll("", "", true);
...
void UDiscordObject::StopDiscordTimer()
{
	...
	activity.GetTimestamps().SetEnd(NULL); 
	activity.GetTimestamps().SetStart(NULL); //FDateTime::UtcNow().ToUnixTimestamp());
	if (core)
	{
		core->ActivityManager().UpdateActivity(activity, [](discord::Result result)
		{
...
void UDiscordObject::UpdateAll(FString NewState, FString NewDetails, const bool bStopTimer /*= false*/)
{
	activity.SetState(TCHAR_TO_UTF8(*NewState));
	activity.SetDetails(TCHAR_TO_UTF8(*NewDetails));
	...
	if (core)
	{
		core->ActivityManager().UpdateActivity(activity, [](discord::Result result) {
			uint8 ResultByte = (uint8)result;
			LogDisplay(FString::Printf(TEXT("UpdateAll Result: %s"), *GetDiscordResultString(static_cast<EDiscordReturnResult>(ResultByte))));
		});
	}
}
